...
fpCam = new CamaraPrimeraPersona(new Vector3(0f, 1.5f, 0f));
      ...
      //ESTE SINO Bounds piso = CrearObjeto("floor", Vector3.zero, Vector3.zero, Vector3.one,new Color(0.96f, 0.87f, 0.70f));
        ...
        //no cambia nada 
       Bounds piso = CrearObjeto("floor", new Vector3(0, -1.5f, 0), Vector3.zero, Vector3.one, new Color(0.96f, 0.70f, 0.45f));
    // Después de crear el piso CREO EL TECHO
    ...
    CrearObjeto("wallB2",
    new Vector3(-2.8f, 0f, 0.5f), // la acercás más al fondo (Z más chico o más grande según tu escena)
    Vector3.zero,
    Vector3.one,
...
void CrearCamara()
{
    ...
    // DESACTIVAR ORBITAL para usar FP
  ...
  Bounds CrearObjeto(string nombreOBJ, Vector3 posicion, Vector3 rotacionGrados, Vector3 escala, Color color)
{
        ...
        //colores por vertice y asigno color uniforme a todos los vertices
    ...
    //bound para saber si el obj esta en pantalla  esta lejos?
...
void Update()
{
    ...
    if (usarFP)
    {
        ...
        // UN INPUT PARA CAMBIAR LA LETRA 

        view = fpCam.CalcularMatrizVista(
            deltaPhi,
            deltaTheta,
            inputAvance,
            inputLateral
    ...
    {
        view = orbital.viewMatrix; // (para activarla despues
    }
